...
export const TunedMassDamperSim: React.FC<SimulationProps> = ({ angle, load , zoom = 1 }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < numSegments; i++) {
      const y1 = i * dy;
      const y2 = (i + 1) * dy;
      const x1 = getDeflectedX(y1);
      const x2 = getDeflectedX(y2);
      const angle1 = Math.atan2(x2 - x1, dy); 
      const [leftX1, rightX1] = [x1 - (buildingWidth/2) * Math.cos(angle1), x1 + (buildingWidth/2) * Math.cos(angle1)];
      const [leftX2, rightX2] = [x2 - (buildingWidth/2) * Math.cos(angle1), x2 + (buildingWidth/2) * Math.cos(angle1)];
      ctx.beginPath();
      ctx.moveTo(leftX1 * scale, -y1 * scale);
      ctx.lineTo(leftX2 * scale, -y2 * scale);
      ctx.lineTo(rightX2 * scale, -y2 * scale);
      ctx.lineTo(rightX1 * scale, -y1 * scale);
      ctx.closePath();
    ...
    const topY = buildingHeight;
    const topX = getDeflectedX(topY);
    ctx.save();
    ctx.translate(topX * scale, -topY * scale); 
    ctx.fillStyle = 'rgba(15, 23, 42, 0.8)';
    ctx.fillRect(-buildingWidth/2 * scale + 5, 5, buildingWidth * scale - 10, 3 * scale);
    ctx.strokeStyle = '#64748b';
    ctx.strokeRect(-buildingWidth/2 * scale + 5, 5, buildingWidth * scale - 10, 3 * scale);
    ctx.beginPath();
...
export const WankelEngineSim: React.FC<SimulationProps> = ({ angle, load , zoom = 1 }) => {
  ...
  useEffect(() => {
    ...
    ctx.setLineDash([]);
    ctx.beginPath();
    ctx.arc(originScreenX, originScreenY, 0.5 * scale, 0, Math.PI * 2);
    ctx.fillStyle = '#ff3366';
    ctx.fill();
    ctx.strokeStyle = '#fff';
    ctx.lineWidth = 2;
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(scX, scY, 0.4 * scale, 0, Math.PI * 2);
    ctx.fillStyle = '#fff';
    ctx.fill();
    const [spX1, spY1] = toScreen(e * Math.cos(3*Math.PI) + R * Math.cos(Math.PI), e * Math.sin(3*Math.PI) + R * Math.sin(Math.PI));
    ctx.beginPath(); ctx.moveTo(spX1, spY1); ctx.lineTo(spX1 - 20, spY1); ctx.strokeStyle = '#fca5a5'; ctx.lineWidth = 4; ctx.stroke();
    ctx.beginPath(); ctx.arc(spX1, spY1, 4, 0, Math.PI*2); ctx.fillStyle = (Math.abs(Math.sin(rotorAngle*3)) > 0.95) ? '#fff' : '#ef4444'; ctx.fill();
    ctx.fillStyle = 'rgba(8, 15, 30, 0.85)';
    ctx.fillRect(30, 150, 260, 110);
    ctx.strokeStyle = 'rgba(255, 51, 102, 0.3)';
    ctx.strokeRect(30, 150, 260, 110);
    ctx.font = '10px monospace';
    ctx.fillStyle = '#ff3366';
    ctx.fillText('SYS :: WANKEL_ROTARY', 40, 170);
    ctx.fillStyle = '#fff';
    const normShaft = ((shaftAngle * 180 / Math.PI) % 360 + 360) % 360;
    const normRotor = ((rotorAngle * 180 / Math.PI) % 360 + 360) % 360;
    ctx.fillText(`ECCENTRIC SHAFT : ${normShaft.toFixed(1)}°`, 40, 195);
    ctx.fillStyle = '#00ff88';
...
export const WattsLinkageSim: React.FC<SimulationProps> = ({ angle, load , zoom = 1 }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const traceRef = useRef<{x: number, y: number}[]>([]);
  useEffect(() => {
    const canvas = canvasRef.current;
...
const SIM_MAP: Record<string, React.FC<{ angle: number; load: number; zoom?: number }>> = {
  'crankshaft': CrankshaftSim,
  'cam-follower': CamFollowerSim,
  'rack-pinion': RackPinionSim,
  'geneva-drive': GenevaDriveSim,
  'planetary-gear': PlanetaryGearSim,
  'harmonic-drive': HarmonicDriveSim,
  'venturi-tube': VenturiTubeSim,
  'i-beam': IBeamStressSim,
  'stirling-engine': StirlingEngineSim,
  'tuned-mass': TunedMassDamperSim,
  'wankel-engine': WankelEngineSim,
  'quick-return': QuickReturnSim,
  'escapement': EscapementSim,
  'scissor-lift': ScissorLiftSim,
  'watts-linkage': WattsLinkageSim,
  'oldham-coupling': OldhamCouplingSim,
  'four-bar-linkage': FourBarLinkageSim,
  'inverted-pendulum': InvertedPendulumSim,
  'mass-spring-damper': MassSpringDamperSim,
  'truss-model': TrussModelSim,
  'scotch-yoke': ScotchYokeSim,
  'external-gear-pump': ExternalGearPumpSim,
  'otto-cycle': OttoCycleSim,
  'peaucellier-lipkin': PeaucellierLipkinSim,
  'sliding-vane-pump': SlidingVanePumpSim,
  'tesla-turbine': TeslaTurbineSim,
  'archimedes-screw': ArchimedesScrewSim,
  'pratt-truss': PrattTrussSim,
  'cantilever-beam': CantileverBeamSim,
  'euler-bernoulli': EulerBucklingSim,
  'carnot-cycle': CarnotCycleSim,
  'strandbeest': StrandbeestSim,
  'klann-linkage': KlannLinkageSim,
  'cvt-transmission': CVTTransmissionSim,
  'swashplate': SwashplateSim,
  'centrifugal-governor': CentrifugalGovernorSim,
